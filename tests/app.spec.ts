...
test('persiste notas, movimiento, diario y gastos', async ({ page, isMobile }) => {
  ...
  if (isMobile) await page.getByRole('button', { name: 'Mover a En progreso' }).click();
  else await page.locator('.note').dragTo(page.locator('.column').nth(1));
