...
export type Note={id:string;text:string;color:string,status:Status,history:{status:Status,at:string}[],archivedAt?:string};
...
export type Expense={id:string,concept:string,cents:number,date?:string,category?:ExpenseCategory,paid?:boolean};
...
export function isData(v:unknown):v is Data{if(!v||typeof v!=='object')return false;const d=v as Data;if(d.version!==1||!Array.isArray(d.notes)||!Array.isArray(d.folders)||!Array.isArray(d.expenses))return false;return d.notes.every(n=>str(n?.id)&&str(n?.text)&&COLORS.includes(n?.color)&&status(n?.status)&&Array.isArray(n?.history)&&n.history.length>0&&n.history.every(h=>status(h?.status)&&date(h?.at))&&(n.archivedAt===undefined||date(n.archivedAt)))&&d.folders.every(f=>str(f?.id)&&str(f?.name)&&Array.isArray(f?.pages)&&f.pages.length>0&&f.pages.every(p=>str(p?.id)&&typeof p?.text==='string'&&date(p?.createdAt)&&plainTextLength(p.text)<=PAGE_LIMIT))&&d.expenses.every(e=>str(e?.id)&&str(e?.concept)&&Number.isSafeInteger(e?.cents)&&e.cents>=0&&(e.date===undefined||isExpenseDate(e.date))&&(e.category===undefined||EXPENSE_CATEGORIES.includes(e.category))&&(e.paid===undefined||typeof e.paid==='boolean'))}
